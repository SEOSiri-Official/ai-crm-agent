    ...
    <div className="flex flex-col lg:flex-row min-h-screen bg-[#020202] text-[#e0e0e0] font-sans selection:bg-blue-600">
      ...
      <aside className="w-full lg:w-80 bg-black border-r border-white/5 p-10 flex flex-col justify-between lg:fixed h-full z-50 shadow-2xl">
        <div className="space-y-12">
          <div className="flex items-center gap-4 cursor-pointer" onClick={()=>window.location.reload()}>
            <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center font-black shadow-[0_0_20px_rgba(37,99,235,0.3)]">S</div>
            <h1 className="text-xl font-black tracking-tighter uppercase italic">SEOSIRI <span className="block text-[8px] text-gray-600 tracking-[0.4em] not-italic">Core v10.0</span></h1>
          </div>
          <nav className="space-y-4 text-[10px] font-bold uppercase tracking-[0.2em] text-gray-500">
            ...
            <button onClick={()=>alert("Voice Interface Active: Listening for Architect Ahmad...")} className="w-full text-left p-4 rounded-xl border border-white/5 flex justify-between items-center opacity-40">Voice Search <span className="w-2 h-2 bg-red-600 rounded-full animate-pulse"></span></button>
          </nav>
        </div>
      ...
      <main className="lg:ml-80 flex-1 p-8 lg:p-20 overflow-y-auto">
        <header className="flex flex-col xl:flex-row justify-between items-start gap-10 mb-20">
          <div>
            <h2 className="text-6xl font-black tracking-tighter uppercase italic leading-none mb-4">System <span className="text-blue-600">Command</span></h2>
            <div className="flex gap-4 p-1 bg-white/5 rounded-full w-fit border border-white/5">
               ...
               <button onClick={()=>setRole("buyer")} className={`px-6 py-2 rounded-full text-[9px] font-black uppercase transition-all ${role === 'buyer' ? 'bg-white text-black shadow-lg' : 'text-gray-500'}`}>Buyer Mode</button>
            </div>
          </div>
          <button onClick={runGlobalSync} disabled={loading} className="w-full xl:w-auto bg-blue-600 text-white px-12 py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-[0_0_50px_rgba(37,99,235,0.4)]">
        ...
        {data ? (
              ...
              <div className="grid grid-cols-12 gap-10">
                <section className="col-span-12 xl:col-span-8 bg-zinc-900/30 border border-white/10 p-12 rounded-[4rem] backdrop-blur-3xl relative overflow-hidden group">
                  ...
                  <div className="flex justify-between items-center mb-10 border-b border-white/5 pb-8">
                     <h3 className="text-blue-500 font-black text-[10px] uppercase tracking-[0.5em]">Deep Intent Report</h3>
                     <div className="flex gap-4">
                        <button onClick={()=>window.print()} className="text-[9px] font-black text-gray-500 hover:text-white uppercase tracking-widest">Download</button>
                        <button onClick={()=>handleAction("Report", data.intelligence.report)} className="text-[9px] font-black text-gray-500 hover:text-white uppercase tracking-widest">Share ID: {data.intelligence.shareId.substring(0,8)}</button>
                     </div>
                  </div>
                ...
                <div className="col-span-12 xl:col-span-4 bg-blue-600 p-12 rounded-[4rem] text-center flex flex-col justify-center shadow-3xl">
                    ...
                    <p className="mt-8 text-[10px] font-bold uppercase tracking-widest border border-white/20 inline-block px-6 py-2 rounded-full mx-auto italic">Market Gap Identified</p>
                </div>
              </div>
            )}
            ...
            {tab === "journey" && (
              <div className="space-y-6">
                ...
                {data.intelligence.leads.map((l: any, i: number) => (
                  <div key={i} className="bg-zinc-900/40 border border-white/5 p-10 rounded-[3rem] flex flex-col xl:flex-row justify-between items-center gap-10 hover:border-blue-600/30 transition-all group">
                    <div className="text-center xl:text-left">
                       <p className="font-black text-2xl mb-1 text-white">{l.name}</p>
                       <p className="text-[9px] text-gray-500 uppercase font-bold tracking-widest italic">Connected via Notion</p>
                    </div>
                    ...
                    <div className="flex gap-4">
                       ...
                       <button onClick={()=>handleAction("Social Script", data.intelligence.report)} className="bg-zinc-800 text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase hover:bg-white hover:text-black transition-all">Social Connect</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="h-[40vh] flex flex-col items-center justify-center opacity-20 text-center space-y-6">
             ...
             <p className="text-[10px] font-black uppercase tracking-[1em] animate-pulse italic text-blue-500">Establishing SEOSIRI Core Connection...</p>
          </div>
        )}
        ...
        <footer className="mt-40 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 opacity-40 text-[9px] font-black uppercase tracking-[0.5em]">
           <div className="flex gap-10"><span>GEO RANKING</span><span>AEO COMPLIANT</span><span>SaaS/PaaS/IaaS</span></div>
