...
export default function SeosiriGlobalMainframe() {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-[#050505] text-[#f2f2f2] font-sans selection:bg-blue-600">
      ...
      <aside className="w-full lg:w-80 bg-black border-r border-white/5 p-10 flex flex-col justify-between lg:fixed h-full z-50">
        <div className="space-y-12">
          <div className="flex items-center gap-4 cursor-pointer" onClick={()=>window.location.reload()}>
            <div className="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center font-black shadow-2xl text-white">S</div>
            <h1 className="text-xl font-black tracking-tighter uppercase italic">SEOSIRI <span className="block text-[8px] text-gray-500 not-italic font-bold tracking-widest uppercase">Global Core</span></h1>
          </div>
        ...
        <div className="pt-8 border-t border-white/5">
           ...
           <p className="text-[8px] text-blue-500 uppercase font-black tracking-tighter">IaaS / PaaS / SaaS Active</p>
        </div>
      </aside>
      ...
      <main className="lg:ml-80 flex-1 p-8 lg:p-20 overflow-y-auto">
        <header className="flex flex-col xl:flex-row justify-between items-start gap-10 mb-20">
          ...
          <div className="flex gap-4">
             <button onClick={()=>window.open('/api/auth/notion')} className="bg-zinc-900 px-8 py-4 rounded-2xl font-black text-[9px] uppercase border border-white/10 hover:bg-white hover:text-black transition-all">Link CRM</button>
             <button onClick={runGlobalSync} disabled={loading} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-[0_0_40px_rgba(37,99,235,0.4)]">
          ...
          <div className="animate-in fade-in slide-in-from-bottom-5 duration-1000 space-y-16">
            ...
            {tab === "journey" && (
              <div className="space-y-6">
                <h3 className="text-gray-500 font-black text-[10px] uppercase tracking-widest px-4 mb-4">Unified Journey Management</h3>
                {data.intelligence.leads.map((l: any, i: number) => (
                  <div key={i} className="bg-zinc-900/40 border border-white/5 p-10 rounded-[3rem] flex flex-col xl:flex-row justify-between items-center gap-10 hover:border-blue-600/30 transition-all group">
                    ...
                    <div className="flex-1 w-full h-1 bg-white/5 rounded-full overflow-hidden relative">
                        <div className="absolute h-full bg-blue-600 shadow-[0_0_15px_rgba(37,99,235,0.5)]" style={{ width: `${80 + i}%` }}></div>
                    </div>
                    <div className="flex gap-4">
        ...
        <footer className="mt-40 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 opacity-40 text-[9px] font-black uppercase tracking-[0.5em]">
           <div className="flex gap-10"><span>Sitemap Ready</span><span>AEO Compliance</span><span>GDPR Policy</span></div>
