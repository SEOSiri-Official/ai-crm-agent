...
export default function SeosiriGlobalMainframe() {
  ...
  const copyToClipboard = (text: string) => {
    ...
    alert("Architect Ahmad: Intelligence Protocol Copied.");
  };
      ...
      <aside className="w-full lg:w-80 bg-black border-r border-white/5 p-10 flex flex-col justify-between lg:fixed h-full z-50">
        <div className="space-y-12">
          <div className="flex items-center gap-4 cursor-pointer" onClick={()=>window.location.reload()}>
            ...
            <h1 className="text-xl font-black tracking-tighter uppercase italic">SEOSIRI <span className="block text-[8px] text-gray-600 not-italic uppercase font-bold tracking-widest">Core v10.0</span></h1>
          </div>
          <nav className="space-y-4 text-[10px] font-bold uppercase tracking-widest text-gray-500">
            <button onClick={()=>setActiveTab("market")} className={`w-full text-left p-4 rounded-xl transition-all ${activeTab==='market'?'bg-blue-600 text-white shadow-xl':'hover:bg-white/5'}`}>Market Intelligence</button>
            ...
            <button onClick={()=>alert("Voice Active: Listening for Ahmad's Command...")} className="w-full text-left p-4 rounded-xl border border-white/5 flex justify-between items-center opacity-40">Voice Command <span className="w-2 h-2 bg-red-600 rounded-full animate-pulse"></span></button>
          </nav>
        </div>
        <div className="pt-8 border-t border-white/5 flex items-center gap-3">
           ...
           <p className="text-[9px] text-gray-600 uppercase tracking-widest font-bold">Architect: Momenul Ahmad<br/>Global SaaS/PaaS/IaaS</p>
        </div>
      </aside>
      ...
      <main className="lg:ml-80 flex-1 p-8 lg:p-20 overflow-y-auto">
        <header className="flex flex-col xl:flex-row justify-between items-start gap-10 mb-20">
          <div>
            ...
            <div className="flex gap-4 p-1 bg-white/5 rounded-full w-fit">
               <button onClick={()=>setRole("seller")} className={`px-8 py-2 rounded-full text-[9px] font-black uppercase transition-all ${role === 'seller' ? 'bg-white text-black shadow-lg' : 'text-gray-500'}`}>Seller Mode</button>
               <button onClick={()=>setRole("buyer")} className={`px-8 py-2 rounded-full text-[9px] font-black uppercase transition-all ${role === 'buyer' ? 'bg-white text-black shadow-lg' : 'text-gray-500'}`}>Buyer Mode</button>
            </div>
          </div>
        ...
        {data ? (
              ...
              <div className="grid grid-cols-12 gap-10">
                <section className="col-span-12 xl:col-span-8 bg-zinc-900/30 border border-white/10 p-12 rounded-[4rem] backdrop-blur-3xl relative overflow-hidden group">
                  ...
                  <div className="flex justify-between items-center mb-10 border-b border-white/5 pb-8">
                     <h3 className="text-blue-500 font-black text-[10px] uppercase tracking-[0.5em]">Deep Intent & Gap Analysis</h3>
                     <div className="flex gap-4">
                        ...
                        <button onClick={()=>copyToClipboard(data.intelligence.report)} className="text-[9px] font-black text-gray-500 hover:text-white border border-white/10 px-4 py-2 rounded-lg">Copy Report</button>
                     </div>
                  </div>
          ...
          <div className="h-[40vh] flex flex-col items-center justify-center opacity-20 text-center space-y-6">
             ...
             <p className="text-[10px] font-black uppercase tracking-[1em] italic text-blue-500">Establishing SEOSIRI Core Connection...</p>
          </div>
        )}
