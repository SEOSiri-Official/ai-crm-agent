...
export default function SeosiriGlobalEngine() {
  ...
  const copyStrategy = (text: string) => {
    ...
    alert("Industrial Protocol Copied to Clipboard.");
  };
      ...
      <aside className="w-full lg:w-80 bg-black border-r border-white/5 p-10 flex flex-col justify-between lg:fixed h-full z-50">
        <div className="space-y-12">
          <div className="flex items-center gap-4 cursor-pointer" onClick={()=>window.location.reload()}>
            ...
            <h1 className="text-xl font-black tracking-tighter uppercase italic">SEOSIRI <span className="block text-[8px] text-gray-500 not-italic uppercase font-bold tracking-widest">Global Mainframe</span></h1>
          </div>
          <nav className="space-y-4 text-[10px] font-bold uppercase tracking-widest text-gray-500">
            <button onClick={()=>setTab("market")} className={`w-full text-left p-4 rounded-xl transition-all ${tab==='market'?'bg-blue-600 text-white shadow-xl':'hover:bg-white/5'}`}>Market Performance</button>
        ...
        <div className="pt-8 border-t border-white/5">
           ...
           <p className="text-[9px] text-blue-500 uppercase font-black">info@seosiri.com</p>
        </div>
      </aside>
      ...
      <main className="lg:ml-80 flex-1 p-8 lg:p-20 overflow-y-auto">
        <header className="flex flex-col xl:flex-row justify-between items-start gap-10 mb-20">
          ...
          <div className="flex gap-4">
             ...
             <button onClick={triggerGlobalSync} disabled={loading} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:scale-105 transition-all shadow-2xl">
        ...
        {data ? (
          <div className="animate-in fade-in slide-in-from-bottom-5 duration-1000 space-y-16">
            {tab === "market" && (
              <div className="grid grid-cols-12 gap-10">
                <section className="col-span-12 xl:col-span-8 bg-zinc-900/30 border border-white/10 p-10 rounded-[4rem] backdrop-blur-3xl relative overflow-hidden group">
                  <div className="absolute top-0 right-0 p-10 opacity-5 text-9xl font-black italic text-blue-600">AI</div>
                  <div className="flex justify-between items-center mb-10 border-b border-white/5 pb-8">
                     <h3 className="text-blue-500 font-black text-[10px] uppercase tracking-[0.5em]">Real-Time Strategic Intel</h3>
                     <button onClick={()=>copyStrategy(data.intelligence.report)} className="text-[9px] font-black text-gray-500 hover:text-white uppercase tracking-widest border border-white/10 px-4 py-2 rounded-lg">Copy Strategy</button>
                  </div>
                ...
                <div className="col-span-12 xl:col-span-4 bg-blue-600 p-12 rounded-[4rem] text-center flex flex-col justify-center shadow-3xl">
                    <p className="text-white/60 text-[10px] font-black uppercase tracking-widest mb-4 italic">Intent Probability</p>
            ...
            {tab === "journey" && (
              <div className="space-y-6">
                <h3 className="text-gray-500 font-black text-[10px] uppercase tracking-widest px-4 mb-4 text-center">Autonomous Journey Timeline</h3>
                {data.intelligence.leads.map((l: any, i: number) => (
                  <div key={i} className="bg-zinc-900/40 border border-white/5 p-10 rounded-[3rem] flex flex-col xl:flex-row justify-between items-center gap-10 hover:border-blue-600/30 transition-all">
                    <div>
                       ...
                       <p className="text-[9px] text-gray-500 uppercase font-bold tracking-widest italic">Source: Notion CRM</p>
                    </div>
                    ...
                    <div className="flex gap-4">
                       ...
                       <button onClick={()=>copyStrategy(data.intelligence.report)} className="bg-zinc-800 text-white px-8 py-3 rounded-xl font-black text-[10px] uppercase hover:bg-white hover:text-black transition-all">Social Connect</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="h-[40vh] flex flex-col items-center justify-center opacity-20 text-center space-y-6">
             ...
             <p className="text-[10px] font-black uppercase tracking-[1em] animate-pulse italic text-blue-500">Connecting SEOSIRI Mainframe...</p>
          </div>
        )}
        ...
        <footer className="mt-40 pt-10 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-8 opacity-40 text-[9px] font-black uppercase tracking-[0.5em]">
           <div className="flex gap-10"><span>Sitemap Ready</span><span>AEO Compliance</span><span>GEO Optimized</span></div>
